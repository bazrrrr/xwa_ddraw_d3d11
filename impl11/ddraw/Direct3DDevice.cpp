...
void Direct3DDevice::UpdatePixelShader(ID3D11DeviceContext *context, Direct3DTexture *texture)
{
	...
	else
	{
		...
		if (g_config.isXvT && texture->_surface->_pixelFormat.dwRGBAlphaBitMask == 0 &&
		    texture->_surface->hasColorKey) pixelShader = this->_deviceResources->_pixelShaderAtestDiscardBlack;
	}
